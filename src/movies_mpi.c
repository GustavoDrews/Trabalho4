...
// Split simples baseado em delimitador (não trata aspas)
...
// Tenta inserir um filme no top 10 local
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        if (rank == 0) {
            fprintf(stderr, "Uso: mpirun -np <P> ./build/movies_mpi data/full_top_rated_movies.csv\n");
        }
        MPI_Finalize();
        return 1;
    }
    ...
    if (rank == 0) {
        printf("Colunas detectadas: lang=%d pop=%d rating=%d original_title=%d\n",
               idx_lang, idx_pop, idx_rating, idx_original_title);
    }
    ...
    if (rank == 0) {
        ...
        printf("=== TOP 10 filmes PT (por vote_average) ===\n");
